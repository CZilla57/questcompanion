...
- [x] **Campaign thread:** map the user's active questline/campaign arc to a chapter; the DM writes short, grounded narration that references **real** completed quests and kingdom growth ("the Forge's east tunnel is clear; the Athenaeum's lanterns relit"). Never invents work the user didn't do. *(Grounds on real completed/planned quest titles, the running campaign's current chapter beat, AND kingdom growth — derived exactly from today's completed tasks' base `points` per kingdom vs. the current lifetime, so a tier advance is only ever reported when it truly crossed today. No new ledger needed.)*
- [x] **Daily beats:** `GET /dm/beat?kind=morning|camp` — a **morning quest board** (today's planned quests framed as the day ahead) and an **evening make-camp** (the day's wins). Cached per (user, day, kind); returns `{ beat: null }` when the day has nothing real to narrate. **Rendered on iOS** (Today card, verified live) **and web** (Now card). Verified end-to-end against the deployed server.
- [x] **Session recap:** folded into `weekly-recap.ts` — the recap prompt + fallback now speak in the DM's "session summary" voice, keeping every anti-shame rule.
...
- [x] **Long rest:** the evening reflection ("make camp") now restores hero vitality (`lastFedAt := now`) inside its first-answer XP claim — once per day, race-safe, upside-only.
- [x] **Gate:** exhaustive unit tests — `dungeon-master.test.ts` (strengths-only fact-builder, tier-crossing growth, substance gate) + `ai/dungeon-master.test.ts` (no-fabrication rejection, guilt rejection, deterministic grounded fallback, model-vs-fallback routing); recap DM-voice test. Rendered + verified on iOS (live) and web. **Phase 3 complete.**
