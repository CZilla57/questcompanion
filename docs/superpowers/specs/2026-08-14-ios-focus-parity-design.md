...
1. **Pomodoro engine location — extract to a shared workspace lib `@workspace/pomodoro`.**
   ...
   It has three consumers (web `focus.tsx`, web `lib/body-double-countdown.ts`, and
   mobile); a single source of truth prevents drift of correctness-critical timer math.
...
Repoint the two web consumers and add the mobile dependency:

- `artifacts/focusquest/src/pages/focus.tsx`: import from `@workspace/pomodoro`
  instead of `@/lib/pomodoro`.
- `artifacts/focusquest/src/lib/body-double-countdown.ts`: same repoint.
- Delete `artifacts/focusquest/src/lib/pomodoro.ts` and its co-located test (moved).
- `artifacts/focusquest-mobile/package.json`: add `"@workspace/pomodoro": "workspace:*"`.
...
- `artifacts/focusquest/src/pages/focus.tsx` — repoint pomodoro import.
- `artifacts/focusquest/src/lib/body-double-countdown.ts` — repoint pomodoro import.
- `pnpm-lock.yaml` — regenerated by `pnpm install` after the new package + dep.
