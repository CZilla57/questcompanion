...
- **Consumption caveat (discovered during implementation):** orval v8 generates `useTranscribeAudio` with a `Blob` body type, but its fetch call does `body: JSON.stringify(blob)` — which serializes a Blob to the literal string `"{}"`, silently discarding the audio (empirically confirmed; declaring `application/octet-stream` instead does not change this — only `text/plain`/form-data skip stringification in orval v8.9.1). The generated hook is therefore **not used**. Instead, the frontend calls `customFetch` (re-exported from `@workspace/api-client-react`) directly inside a plain `useMutation`, with `headers: { "content-type": blob.type }` and the raw `Blob` as body. The OpenAPI operation stays the documented source of truth and the generated `TranscribeResult` type still types the response; only the request-construction layer is hand-written for this one binary endpoint. Do not "clean up" the unused generated hook — it regenerates from the spec, and the wrapper carries a comment explaining why it exists.
